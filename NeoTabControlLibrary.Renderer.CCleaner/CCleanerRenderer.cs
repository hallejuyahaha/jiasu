        ...
        public override void OnRendererTabPageItem(Graphics gfx, Rectangle tabPageItemRct, string tabPageText, int index, CommonObjects.ButtonState btnState)
        {
            ...
            switch (tabPageText.ToUpperInvariant())
            {
                ...
                case "&OUTPUT":
                    ...
                    break;
                case "系统":
                    if (btnState == CommonObjects.ButtonState.Pressed)
                    {
                        itemBitmap = Resources.system;
                    }else
                    {
                        itemBitmap = Resources.system_w;
                    }
                    break;
                case "选项":
                    if (btnState == CommonObjects.ButtonState.Pressed)
                    {
                        itemBitmap = Resources.option;
                    }else
                    {
                        itemBitmap = Resources.option_w;
                    }
                    break;
                case "关于":
                    if (btnState == CommonObjects.ButtonState.Pressed)
                    {
                        itemBitmap = Resources.about;
                    }else
                    {
                        itemBitmap = Resources.about_w;
                    }
                    break;
                default:
                    itemBitmap = Resources.system;
